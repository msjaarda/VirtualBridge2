...
disp("Vous utilisez la version 3.0 du 16/04/2026, Lucas");
disp("-----------------------------------------------------");
a = 1;
...
while b == 0
disp("-----------------------------------------------------");
Prompt = 'Type de pont : 1)Box 2)Twin 3)Multi 4)Slab 5)Flamatt \n';
Entry = input(Prompt);
...
Donnee.BridgeType = {'Box';'Twin';'Multi';'Slab';'Flamatt'};
...
if size(Fields,1)>2
Fields = [Fields(end-1:end,:);Fields(1:end-2,:)];
end
