...
for g = 1:height(BaseData)
    ...
    %PDs = PDs(PDs.DTS > datetime(2019,10,17) & PDs.DTS < datetime(2019,10,19),:);
    ...
    for z = 1:max(PDsy.Group)
        ...
        % We also must subdivide TrLineUp
        %TrLineUpSub = TrLineUp(TrLineUp(:,6) == z,:);
        ...
        while k < BaseData.NumAnalyses(g) & sum(AllTrAxSub,'all') > 0
            ...
            % We don't add padding because it messes with stuff...
%             if BrStInd < 1 | BrStInd + BrLengthInd - 1 > height(AllTrAxSub)
%                 AllTrAxSub(1:BrLengthInd,:) = 0;
%                 AllTrAxSub(end-BrLengthInd:BrLengthInd,:) = 0;
%                 continue
%             end
            ...
            % Get Strip Inds (Call it BrInds when we don't use a strip)
