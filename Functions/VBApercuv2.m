...
TotalLanes = Lane.Sites.NumLanes; % OK
%ALane = Lane.Details.ALANE;
% We need to sort Lane.Details by LANE first... then do this.
Lane.Details = sortrows(Lane.Details,3);

% NEW
Lane.Details = sortrows(Lane.Details,{'ALANE'});
%Lane.Dir = Lane.Dir(SortIndex);

% Get total number of subplots OK
...
for i = 1:TotalLanes
    ...
    q{i} = Q(Q(:,4) == Lane.Details.LANE(i),:); t{i} = T(T.LANE == Lane.Details.LANE(i),:);
...
for j = 1:TotalLanes
    ...
    if j == 1 && isWIM
        ...
        if isfield(Lane,'Details')
            if Lane.Details.NSEW(j) == 4 || Lane.Details.NSEW(j) == 3
    ...
    if ~isempty(q{j})
        ...
        if j > 1 && TotalLanes > 2 && j < TotalLanes
    ...
    if j ~= TotalLanes
        ...
        if Lane.Details.NSEW(j) == Lane.Details.NSEW(j+1)
