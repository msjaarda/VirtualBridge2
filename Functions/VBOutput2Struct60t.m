function [VBResults] = VBOutput2Struct60t(Folder_Name)
...
% Make a table
VBResults.Weekly = array2table(zeros(0,11), 'VariableNames',{'Type','SubType','Width','Layout','Support','Trans','AE','Traffic','Span','All','ClassOW'});
    ...
    for k = 1:length(ia)
        ...
        elseif strcmp(OInfo(i).BaseData.AnalysisType,'WIM')
            ...
            if OInfo(i).SimStop
                ...
                T11 = array2table(OInfo(i).AQ.ClassOW.Weekly(ic == k)','VariableNames',{'ClassOW'});
                %T12 = array2table(OInfo(i).AQ.Class.Weekly(ic == k)','VariableNames',{'Class'});
                VBResults.Weekly = [VBResults.Weekly; T1 T2 T3 T4 T5 T6 T7 T8 T9 T10 T11];
