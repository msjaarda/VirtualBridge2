...
for i = 1:length(OInfo)
    ...
    for k = 1:length(OInfo(i).ILData)
        ...
        if contains(Section,'Box') || contains(Section,'Twin')
            ...
            try MAT.(Section).(Config).(Dist).(AE)(end+1,:) = array2table(NaN(1,length(ColumnNames)),'VariableNames',ColumnNames);
        ...
        elseif contains(Section,'Multi')
            ...
            SpanDiv = 10;
            SpanDivShift = 10;
            try  MAT.(Section).(Config).(Dist).(PLoc).(AE)(end+1,:) = array2table(NaN(1,length(ColumnNames)),'VariableNames',ColumnNames);
            ...
            MAT.(Section).(Config).(Dist).(PLoc).(AE).E((Span-SpanDivShift)/SpanDiv) = OInfo(i).ESIA.Total(k);
            MAT.(Section).(Config).(Dist).(PLoc).(AE).Eq((Span-SpanDivShift)/SpanDiv) = OInfo(i).ESIA.Eq(k);
            MAT.(Section).(Config).(Dist).(PLoc).(AE).EQ1((Span-SpanDivShift)/SpanDiv) = OInfo(i).ESIA.EQ(1,k);
            MAT.(Section).(Config).(Dist).(PLoc).(AE).EQ2((Span-SpanDivShift)/SpanDiv) = OInfo(i).ESIA.EQ(2,k);
            ...
            MAT.(Section).(Config).(Dist).(PLoc).(AE).E(Span/SpanDiv) = OInfo(i).ESIA.Total(k);
            MAT.(Section).(Config).(Dist).(PLoc).(AE).Eq(Span/SpanDiv) = OInfo(i).ESIA.Eq(k);
            MAT.(Section).(Config).(Dist).(PLoc).(AE).EQ1(Span/SpanDiv) = OInfo(i).ESIA.EQ(1,k);
            MAT.(Section).(Config).(Dist).(PLoc).(AE).EQ2(Span/SpanDiv) = OInfo(i).ESIA.EQ(2,k);
